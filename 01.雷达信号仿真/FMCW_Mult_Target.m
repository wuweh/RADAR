...
%对距离维FFT结果再次做FFT，得到速度维频谱
for n=1:range_fft
    fft2(n,:) = abs(fft(abs(fft1(n,:)),dopple_fft));
end
...
subplot(2,2,3)
for n=1:64
    plot(range(1:range_fft/2),abs(fft1(1:range_fft/2,n)));
    hold on
end


%速度维频谱图
dopple_f = ((1:dopple_fft)-1-33)*1/tm/dopple_fft*lambda/2/3.6;
for n = 1:10   
    subplot(2,2,4)
...
figure;
mesh(dopple_f(1:dopple_fft/2),range(1:range_fft/2),fft2(1:range_fft/2,1:dopple_fft/2));
view(3);
ylabel('distance:m');

...
% basic parameters
model.x_dim= 4;   %dimension of state vector  ×´Ì¬¾ØÕóÎ¬Êý
model.z_dim= 2;   %dimension of observation vector ¹Û²â¾ØÕóÎ¬Êý

% dynamical model parameters (CV model)
model.T= 1;                                     %sampling period                    ²ÉÑùÖÜÆÚ
model.A0= [ 1 model.T; 0 1 ];                         %transition matrix       ×ªÒÆ¾ØÕó              
model.F= [ model.A0 zeros(2,2); zeros(2,2) model.A0 ];                      %×ªÒÆ¾ØÕó
model.B0= [ (model.T^2)/2; model.T ];                                           % 
model.B= [ model.B0 zeros(2,1); zeros(2,1) model.B0 ];   %¹Û²â¾ØÕó
model.sigma_v = 5;
